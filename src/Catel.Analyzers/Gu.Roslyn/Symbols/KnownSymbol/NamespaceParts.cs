...
public sealed class NamespaceParts
{
    ...
    public override bool Equals(object? obj)
    {
        ...
        return obj is NamespaceParts namespaceParts &&
               Equals(namespaceParts);
    }

    /// <inheritdoc />
    public override int GetHashCode()
    {
        return parts[parts.Count - 1].GetHashCode();
    }
    ...
    {
        return Matches(nameSyntax, parts.Count - 1);
    }
    ...
    private bool Matches(NameSyntax nameSyntax, int index)
    {
        ...
        {
            return index == 0 &&
                   identifier.Identifier.ValueText == parts[0];
        }
        ...
        if (nameSyntax is QualifiedNameSyntax qns)
        {
            ...
            if (qns.Right.Identifier.ValueText != parts[index])
            {
                return false;
            }

            return Matches(qns.Left, index - 1);
        }
