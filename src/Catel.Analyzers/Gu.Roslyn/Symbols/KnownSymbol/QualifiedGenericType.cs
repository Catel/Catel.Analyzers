...
public class QualifiedGenericType : QualifiedType
{
              ...
              null)
    {
        if (fullName.IndexOf("[", StringComparison.Ordinal) is int i &&
            i > 0)
        {
            metaDataName = fullName.Substring(0, i);
        }
        else
        {
    ...
    public override ITypeSymbol? GetTypeSymbol(Compilation compilation)
    {
        ...
        if (compilation.GetTypeByMetadataName(metaDataName) is { } typeDef &&
        ...
        {
            args = new ITypeSymbol[TypeArguments.Length];
            for (var i = 0; i < TypeArguments.Length; i++)
            {
                if (TypeArguments[i].GetTypeSymbol(compilation) is { } arg)
                {
                    args[i] = arg;
                }
                else
