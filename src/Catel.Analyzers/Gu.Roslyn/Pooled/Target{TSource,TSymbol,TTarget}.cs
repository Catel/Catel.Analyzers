    ...
    where TTarget : SyntaxNode
{
    ...
    public TTarget? TargetNode => Declaration;
    ...
    public bool Equals(Target<TSource, TSymbol, TTarget> other)
    {
        return EqualityComparer<TSource>.Default.Equals(Source, other.Source) &&
               EqualityComparer<TSymbol>.Default.Equals(Symbol, other.Symbol) &&
               EqualityComparer<TTarget?>.Default.Equals(Declaration, other.Declaration);
    }
    ...
    public override int GetHashCode()
    {
        unchecked
        {
            var hashCode = EqualityComparer<TSource>.Default.GetHashCode(Source);
            hashCode = (hashCode * 397) ^ EqualityComparer<TSymbol>.Default.GetHashCode(Symbol);
            hashCode = (hashCode * 397) ^ (Declaration is null ? 0 : EqualityComparer<TTarget>.Default.GetHashCode(Declaration));
            return hashCode;
        }
    }
}
