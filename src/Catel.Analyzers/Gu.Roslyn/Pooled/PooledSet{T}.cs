...
#pragma warning restore IDISP015 // Member should not return created and cached instance.
    ...
    /// <returns>True if the item was added.</returns>
    public bool SetEquals(IEnumerable<T> items) => inner.SetEquals(items);
    ...
    private void ThrowIfDisposed()
    {
        if (refCount <= 0)
        {
            Debug.Assert(refCount == 0, $"{nameof(this.ThrowIfDisposed)} set.refCount == {refCount}");
