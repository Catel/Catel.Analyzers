...
public sealed class Recursion : IDisposable
{
    ...
    {
        if (visited.Add((caller, line, node)) &&
            EffectiveSymbol<IMethodSymbol>(node) is { } symbol)
        {
            _ = symbol.TrySingleDeclaration(CancellationToken, out CSharpSyntaxNode? declaration);
    ...
    {
        return Target<InvocationExpressionSyntax, IMethodSymbol, SyntaxNode>(node, caller, line);
    }
    ...
    {
        if (visited.Add((caller, line, node)) &&
            SemanticModel.TryGetSymbol(node, CancellationToken, out var symbol))
        {
            _ = symbol.TrySingleDeclaration(CancellationToken, out ConstructorDeclarationSyntax? declaration);
...
#pragma warning restore RS0026 // Do not add multiple public overloads with optional parameters
    ...
    {
        if (visitedForType.Add((caller, line, node)) &&
            SemanticModel.TryGetType(node, CancellationToken, out var symbol))
        {
            _ = symbol.TrySingleDeclaration(CancellationToken, out TypeDeclarationSyntax? declaration);
            return AnalyzerExtensions.Target.Create(node, symbol, declaration);
        }
    ...
    public Target<ArgumentSyntax, IParameterSymbol, BaseMethodDeclarationSyntax>? Target(ArgumentSyntax node, [CallerMemberName] string? caller = null, [CallerLineNumber] int line = 0)
    {
        if (visited.Add((caller, line, node)) &&
            ...
            EffectiveSymbol<IMethodSymbol>(parent) is { } method &&
            method.TryFindParameter(node, out var symbol))
        {
            _ = method.TrySingleDeclaration(CancellationToken, out BaseMethodDeclarationSyntax? declaration);
            return AnalyzerExtensions.Target.Create(node, symbol, declaration);
        }
    ...
    {
        if (visited.Add((caller, line, node)) &&
            SemanticModel.TryGetSymbol(node, CancellationToken, out ILocalSymbol? symbol))
        {
            _ = symbol.TryGetScope(CancellationToken, out var declaration);
            return AnalyzerExtensions.Target.Create(node, symbol, declaration);
        }
    ...
    {
        if (visited.Add((caller, line, node)) &&
            SemanticModel.TryGetSymbol(node, CancellationToken, out ITypeSymbol? symbol))
        {
            _ = symbol.TrySingleDeclaration(CancellationToken, out TypeDeclarationSyntax? declaration);
            return AnalyzerExtensions.Target.Create(node, symbol, declaration);
        }
    ...
    {
        return Target<IdentifierNameSyntax, ISymbol, SyntaxNode>(node, caller, line);
    }
    ...
    public Target<ExpressionSyntax, ISymbol, SyntaxNode>? Target(ExpressionSyntax node, [CallerMemberName] string? caller = null, [CallerLineNumber] int line = 0)
    {
        return node switch
        {
            ...
            when Target(invocation, caller, line) is { } temp
            => AnalyzerExtensions.Target.Create(temp.Source, temp.Symbol, (SyntaxNode?)temp.Declaration),
            _ => Target<ExpressionSyntax, ISymbol, SyntaxNode>(node, caller, line),
        };
    }
    ...
    {
        if (visited.Add((caller, line, node)) &&
            EffectiveSymbol<IPropertySymbol>(node) is { } property &&
            property is { SetMethod: { Parameters.Length: 1 } set })
        {
            _ = set.TrySingleDeclaration(CancellationToken, out AccessorDeclarationSyntax? declaration);
    ...
    public Target<ExpressionSyntax, IMethodSymbol, SyntaxNode>? PropertyGet(ExpressionSyntax node, [CallerMemberName] string? caller = null, [CallerLineNumber] int line = 0)
    {
        ...
        {
            _ = get.TrySingleDeclaration(CancellationToken, out SyntaxNode? declaration);
    ...
    public Target<ExpressionSyntax, IMethodSymbol, SyntaxNode>? MethodGroup(ExpressionSyntax node, [CallerMemberName] string? caller = null, [CallerLineNumber] int line = 0)
    {
        if (visited.Add((caller, line, node)) &&
            EffectiveSymbol<IMethodSymbol>(node) is { } symbol)
        {
            _ = symbol.TrySingleDeclaration(CancellationToken, out SyntaxNode? declaration);
            return AnalyzerExtensions.Target.Create(node, symbol, declaration);
        }
    ...
    {
        if (visited.Add((caller, line, node)) &&
            EffectiveSymbol<TSymbol>(node) is { } symbol)
        {
            _ = symbol.TrySingleDeclaration(CancellationToken, out TDeclaration? declaration);
            return AnalyzerExtensions.Target.Create(node, symbol, declaration);
        }
    ...
    private TSymbol? EffectiveSymbol<TSymbol>(SyntaxNode node)
        where TSymbol : class, ISymbol
    {
        if (SemanticModel.TryGetSymbol<TSymbol>(node, CancellationToken, out var symbol))
        {
