    ...
    where T : ExecutionWalker<T>
{
    ...
    /// </summary>
    protected SemanticModel SemanticModel => Recursion.SemanticModel;

    /// <summary>
    /// Gets the containing <see cref="INamedTypeSymbol"/> where the recursion started.
    /// </summary>
    protected INamedTypeSymbol ContainingType => Recursion.ContainingType;

    /// <summary>
    /// Gets the <see cref="CancellationToken"/>.
    /// </summary>
    protected CancellationToken CancellationToken => Recursion.CancellationToken;
...
#pragma warning restore IDISP008 // Don't assign member with injected and created disposables.
    ...
    public override void VisitConstructorDeclaration(ConstructorDeclarationSyntax node)
    {
        ...
        if (SearchScope != SearchScope.Member &&
            node.Initializer is null &&
            SemanticModel.TryGetSymbol(node, CancellationToken, out var ctor) &&
    ...
    public override void VisitConstructorInitializer(ConstructorInitializerSyntax node)
    {
        ...
        switch (SearchScope)
        {
            ...
            case SearchScope.Recursive:
                if (Recursion.Target(node) is { Declaration: { } declaration })
                {
                    Visit(declaration);
                }
                ...
                break;
            default:
                throw new InvalidOperationException($"Not handling member {SearchScope}.");
        }
    }
    ...
    public override void VisitObjectCreationExpression(ObjectCreationExpressionSyntax node)
    {
        ...
        if (Recursion.Target(node.Type) is { Symbol: { } containingType, Declaration: { } containingTypeDeclaration } &&
            ShouldVisit(containingType))
        {
            ...
            if (Recursion.Target(node) is { Symbol: { }, Declaration: { } declaration })
            {
                Visit(declaration);
            }
            ...
            {
                VisitInitializerExpression(objectInitializer);
            }
        }
        ...
        bool ShouldVisit(ITypeSymbol type)
        {
            return SearchScope switch
            {
                ...
                _ => TypeSymbolComparer.Equal(type, ContainingType),
            };
        }
    }
    ...
    public override void VisitImplicitObjectCreationExpression(ImplicitObjectCreationExpressionSyntax node)
    {
        ...
        if (Recursion.TargetType(node) is { Symbol: { } containingType, Declaration: { } containingTypeDeclaration } &&
            ShouldVisit(containingType))
        {
            using (var walker = TypeDeclarationWalker.Borrow(containingTypeDeclaration))
            {
    ...
    public override void VisitInvocationExpression(InvocationExpressionSyntax node)
    {
        ...
        if (TryGetTargetSymbol<InvocationExpressionSyntax, IMethodSymbol, MethodDeclarationSyntax>(node, out var target) &&
    ...
    public override void VisitIdentifierName(IdentifierNameSyntax node)
    {
        ...
        if (TryGetTargetSymbol<IdentifierNameSyntax, IPropertySymbol, PropertyDeclarationSyntax>(node, out var target) &&
            target.Declaration is { } declaration)
        {
            if (IsPropertyGetAndSet(node))
            {
                Visit(declaration.Getter());
                Visit(declaration.Setter());
            }
            else if (IsPropertySet(node))
            {
                Visit(declaration.Setter());
            }
            else if (target.Symbol.TryGetGetMethodDeclaration(CancellationToken, out var getter))
            {
                Visit(getter);
            }
        }
...
#pragma warning restore CA1068 // CancellationToken parameters must come last
    ...
    protected virtual void VisitTypeDeclaration(TypeDeclarationSyntax node)
    {
        ...
        {
            if (SearchScope == SearchScope.Instance &&
    ...
    protected virtual bool IsPropertySet(IdentifierNameSyntax node)
    {
        return node switch
        {
            ...
            _ => SearchScope switch
            {
                ...
                _ => throw new InvalidOperationException($"Unhandled scope: {SearchScope}"),
            },
        };
    }
        ...
        where TDeclaration : SyntaxNode
    {
        target = default;
        if (node is null ||
            SearchScope == SearchScope.Member)
        {
            return false;
        }
        ...
        if (Recursion.Target<TSource, TSymbol, TDeclaration>(node, caller, line) is { Symbol: { } symbol, Declaration: { } } t)
        {
            if (SearchScope == SearchScope.Instance &&
                symbol.IsStatic)
            {
            ...
            if (SearchScope.IsEither(SearchScope.Instance, SearchScope.Type) &&
                ContainingType?.IsAssignableTo(symbol.ContainingType, SemanticModel.Compilation) != true)
            {
                return false;
            }
...
#pragma warning restore SA1401 // Fields must be private
        ...
        public override void VisitFieldDeclaration(FieldDeclarationSyntax node)
        {
            ...
            {
                Initializers.Add(equalsValueClause);
            }
        }
