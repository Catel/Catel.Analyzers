...
public sealed class ObjectCreationWalker : PooledWalker<ObjectCreationWalker>
{
    ...
    /// </summary>
    public IReadOnlyList<BaseObjectCreationExpressionSyntax> ObjectCreations => objectCreations;
    ...
    public void RemoveAll(Predicate<ObjectCreationExpressionSyntax> match) => objectCreations.RemoveAll(s => s is ObjectCreationExpressionSyntax syntax && match(syntax));

    /// <summary>
    /// Filters by <paramref name="match"/>.
    /// </summary>
    /// <param name="match">The predicate for finding items to remove.</param>
    public void RemoveAll(Predicate<BaseObjectCreationExpressionSyntax> match) => objectCreations.RemoveAll(match);
