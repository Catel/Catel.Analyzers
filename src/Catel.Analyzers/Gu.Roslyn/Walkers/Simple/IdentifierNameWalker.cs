...
public sealed class IdentifierNameWalker : PooledWalker<IdentifierNameWalker>
{
    ...
    /// </summary>
    public IReadOnlyList<IdentifierNameSyntax> IdentifierNames => identifierNames;
    ...
    {
        foreach (var candidate in identifierNames)
        {
    ...
    {
        for (var i = identifierNames.Count - 1; i >= 0; i--)
        {
            var candidate = identifierNames[i];
            if (candidate.IsSymbol(symbol, semanticModel, cancellationToken))
            {
                identifierName = candidate;
                return true;
    ...
    public void RemoveAll(Predicate<IdentifierNameSyntax> match) => identifierNames.RemoveAll(match);
