...
public sealed class NullCheckWalker : PooledWalker<NullCheckWalker>
{
    ...
    public bool TryGetFirst(IParameterSymbol parameter, SemanticModel semanticModel, CancellationToken cancellationToken, [NotNullWhen(true)] out ExpressionSyntax? check)
    {
        ...
        foreach (var isPattern in isPatterns)
        {
    ...
    internal void Filter(IParameterSymbol parameter, SemanticModel semanticModel, CancellationToken cancellationToken)
    {
        ...
        {
            var binary = binaryExpressions[i];
            if (Is(binary.Left, parameter, semanticModel, cancellationToken) ||
                Is(binary.Right, parameter, semanticModel, cancellationToken))
            {
                binaryExpressions.RemoveAt(i);
            }
        }

        for (var i = isPatterns.Count - 1; i >= 0; i--)
        {
            if (Is(isPatterns[i], parameter, semanticModel, cancellationToken))
            {
                isPatterns.RemoveAt(i);
            }
        }

        for (var i = invocations.Count - 1; i >= 0; i--)
        {
